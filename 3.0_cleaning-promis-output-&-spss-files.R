...
promis <- read_rds(here("1.data", "2.processed", "2.PROMIS", "promis.rds")) %>% 
  as.data.frame()
rtw_df <- read_spss(file =  here("1.data", "1.original", "RTW COX analysebestand.sav"))
...
rtw_df$PatiëntID <- str_replace_all(rtw_df$PatiëntID, "e", "z")
...
# Recode variables
# Change StatusRTWfull from haven_labelled to a regular factor
# And set factor levels and reference
...
RCT$OK_primair <- as_factor(RCT$OK_primair) %>%
  recode(`1` = "1st primary knee arthroplasty",
         `2` = "2nd primary knee arthroplasty")

RCT$prior_hip <- as_factor(RCT$prior_hip) %>%
  recode(`1` = "Yes, leftside",
         `2` = "Yes, rightside",
         `3` = "Yes, both sides",
         `4` = "No")
...
RCT$living_sit <- as_factor(RCT$living_sit) %>%
  recode(`1` = "Live alone",
         `2` = "Live with partner/family",
         `3` = "Live with kids (without partner)")

RCT$employment <- as_factor(RCT$employment) %>%
  recode(
    `1` = "Salaried",
    `2` = "Self-employed",
    `3` = "Both salaried and self-employed"
    )
